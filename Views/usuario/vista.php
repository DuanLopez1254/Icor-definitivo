...
<p class="display-4 text-center " >Lista de usuarios Inactivos</p>
<br>
...
<thead>
    ...
    <tbody>
    <?php foreach(parent::alls() as $usuario):  ?>
        <tr>
        <td><?= $usuario->id_regis ?></td>
            ...
            <form  action="?controller=usuario&method=updates" method="POST" >
            <input type="hidden" name="id_regis" value="<?= $usuario->id_regis ?>">
            <input type="hidden" name="id_esta" value="1">
            <button type="submit" class="btn btn-success col-md-6">Activar</button>
                    </form>
            </td>
        </tr>
    <?php endforeach; ?>
